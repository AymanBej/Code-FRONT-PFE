...
const TabSites = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const url = "https://localhost:7048/api/FollowedSitesUser/sites";
        ...
        const formattedData = fetchedData.value.map((site) => {
          const lastModifiedDateTime = site.lastModifiedDateTime
            ? format(new Date(site.lastModifiedDateTime), "yyyy-MM-dd HH:mm:ss")
  ...
  const table = useMaterialReactTable({
    ...
    renderTopToolbar: ({}) => {
      return (
        <Toolbar>
          <Typography variant="h6">Liste des sites suivis</Typography>
          <Link to="/sites">
            <IconButton>
